...
def checkFileExistance(filePath):
    try:
        ...
        return False
if checkFileExistance('/content/gdrive/My Drive/BancoDados/UIUC_2_num.dat'):
    import sys
...
# UIUC_2 : lista que contará os dados para estatística dos classificadores
...
UIUC_2_num = np.zeros(20, dtype=np.float64)
UIUC_2_num[0] = best_acc
UIUC_2_num[1] = xensemb
xvencedor=[]
tg = 2
# a seguir salvaremos os melhores scores obtidos
for z in range(3):
  UIUC_2_num[tg] = xval[z]
  
  tg += 1
for z in range(3):
  UIUC_2_num[tg] = xscor1[z]
  
  tg += 1
for z in range(3):
  UIUC_2_num[tg] = et_scor[z]
  
  tg += 1
for z in range(3):
  UIUC_2_num[tg] = es_scor[z]
  
  tg += 1
for z in range(3):
  UIUC_2_num[tg] = mt_scor[z]
  
  tg += 1
for z in range(3):
  UIUC_2_num[tg] = ms_scor[z]
  tg += 1
...
bs_total = as_total - 2
UIUC_2_class = np.empty(bs_total, dtype=np.float64)
for i in range(bs_total):
    if xvencedor[i] == xnome_class[0]:
        UIUC_2_class[i] = float(0.0)
    if xvencedor[i] == xnome_class[1]:
        UIUC_2_class[i] = float(1.0)
    if xvencedor[i] == xnome_class[2]:
        UIUC_2_class[i] = float(2.0)
    if xvencedor[i] == xnome_class[3]:
        UIUC_2_class[i] = float(3.0)
    if xvencedor[i] == xnome_class[4]:
        UIUC_2_class[i] = float(4.0)
    if xvencedor[i] == xnome_class[5]:
        UIUC_2_class[i] = float(5.0)
    if xvencedor[i] == xnome_class[6]:
        UIUC_2_class[i] = float(6.0)
    if xvencedor[i] == xnome_class[7]:
        UIUC_2_class[i] = float(7.0)
    if xvencedor[i] == xnome_class[8]:
        UIUC_2_class[i] = float(8.0)
    if xvencedor[i] == xnome_class[9]:
        UIUC_2_class[i] = float(9.0)
cs_total = as_total - 2
UIUC_2_qtd = np.zeros(18, dtype=np.float64)
UIUC_2_qtd[0]=n_prim
UIUC_2_qtd[1]=n_seg
UIUC_2_qtd[2]=n_ter
UIUC_2_qtd[3]=xn_prim
UIUC_2_qtd[4]=xn_seg
UIUC_2_qtd[5]=xn_ter
UIUC_2_qtd[6]=et_n_prim
UIUC_2_qtd[7]=et_n_seg
UIUC_2_qtd[8]=et_n_ter
UIUC_2_qtd[9]=es_n_prim
UIUC_2_qtd[10]=es_n_seg
UIUC_2_qtd[11]=es_n_ter
UIUC_2_qtd[12]=mt_n_prim
UIUC_2_qtd[13]=mt_n_seg
UIUC_2_qtd[14]=mt_n_ter
UIUC_2_qtd[15]=ms_n_prim
UIUC_2_qtd[16]=ms_n_seg
UIUC_2_qtd[17]=ms_n_ter
pprint(UIUC_2_num)
pprint(UIUC_2_class)
pprint(UIUC_2_qtd)

arq_est_num = "/content/gdrive/My Drive/BancoDados/UIUC_2_num.dat"
arq_est_class = "/content/gdrive/My Drive/BancoDados/UIUC_2_class.dat"
arq_est_qtd = "/content/gdrive/My Drive/BancoDados/UIUC_2_qtd.dat"
UIUC_2_num.tofile(arq_est_num)
UIUC_2_class.tofile(arq_est_class)
UIUC_2_qtd.tofile(arq_est_qtd)

a_num = np.fromfile(arq_est_num)
b_class = np.fromfile(arq_est_class)
c_qtd = np.fromfile(arq_est_qtd)
pprint(a_num == UIUC_2_num)
pprint(b_class == UIUC_2_class)
pprint(c_qtd == UIUC_2_qtd)
